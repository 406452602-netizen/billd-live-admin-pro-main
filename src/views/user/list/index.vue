    ...
    >
      <n-form>
        <n-form-item label="充值金额">
          <n-input-number v-model:value="formWallet.amount"></n-input-number>
        </n-form-item>
        <n-form-item label="备注">
          <n-input
            v-model:value="formWallet.remark"
            type="textarea"
          ></n-input>
        </n-form-item>
      </n-form>
...
import { changeWallet } from '@/api/wallet.ts';
...
const rechargeWallet = () => {
  changeWallet(formWallet.value).then((res) => {
    if (res.code === 200) {
      window.$message.success('充值成功');
      modalWallet.value = false;
...
const updateUser = async (val) => {
  try {
    ...
    const { msg } = await fetchUpdateUser({
      ...val,
