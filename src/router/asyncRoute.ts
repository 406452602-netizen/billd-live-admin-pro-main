import {
  BagHandleOutline,
  BulbOutline,
  ChatbubblesOutline,
  FootstepsOutline,
  HomeOutline,
  ListCircleOutline,
  LockClosedOutline,
  PaperPlaneOutline,
  PersonOutline,
  ShieldOutline,
  WalletOutline,
} from '@vicons/ionicons5';
import { RouteRecordRaw } from 'vue-router';

import {
  DEFAULT_AUTH_INFO,
  DEFAULT_ROLE_INFO,
  ROUTE_ROLES,
  ROUTE_SORT,
} from '@/constant';
import Layout from '@/layout/index.vue';

import { notFoundRoutes } from './notFound';
import { renderIcon } from './utils';

// 默认路由
export const asyncRoutes: RouteRecordRaw[] = [
  {
    path: '/liveManage',
    component: Layout,
    meta: {
      title: 'router.liveManage',
      icon: renderIcon(BulbOutline),
      sort: ROUTE_SORT.live,
      roles: [
        DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value,
        DEFAULT_ROLE_INFO.LIVE_ADMIN.role_value,
      ],
    },
    children: [
      {
        name: 'liveOnline',
        path: '/liveManage/liveOnline',
        component: () => import('@/views/live/liveOnline/list/index.vue'),
        meta: {
          title: 'router.liveOnline',
        },
      },
      {
        name: 'liveUser',
        path: '/liveManage/liveUser',
        component: () => import('@/views/live/liveUser/list/index.vue'),
        meta: {
          title: '在线用户',
        },
      },
      {
        name: 'liveRecord',
        path: '/liveManage/record',
        component: () => import('@/views/live/record/list/index.vue'),
        meta: {
          title: 'router.liveRecord',
        },
      },
      {
        name: 'liveForwardList',
        path: '/liveManage/forwardList',
        component: () => import('@/views/live/forwardList/index.vue'),
        meta: {
          title: 'router.liveForwardList',
          roles: [DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value],
        },
      },
    ],
  },
  {
    path: '/blacklist',
    component: Layout,
    meta: {
      title: '黑名单管理',
      icon: renderIcon(LockClosedOutline),
      sort: ROUTE_SORT.blacklist,
      roles: [
        DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value,
        DEFAULT_ROLE_INFO.LIVE_ADMIN.role_value,
      ],
    },
    children: [
      {
        name: 'blacklistList',
        path: '/blacklist/list',
        component: () => import('@/views/blacklist/list/index.vue'),
        meta: {
          title: '黑名单列表',
        },
      },
      {
        name: 'blacklistCreate',
        path: '/blacklist/create',
        component: () => import('@/views/blacklist/create/index.vue'),
        meta: {
          title: '新增黑名单',
        },
      },
      {
        name: 'blacklistUpdate',
        path: '/blacklist/update',
        component: () => import('@/views/blacklist/update/index.vue'),
        meta: {
          title: '修改黑名单',
          hidden: true,
        },
      },
    ],
  },
  {
    path: '/liveRoomManage',
    component: Layout,
    meta: {
      title: 'router.liveRoomManage',
      icon: renderIcon(HomeOutline),
      sort: ROUTE_SORT.live,
      roles: [
        DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value,
        DEFAULT_ROLE_INFO.LIVE_ADMIN.role_value,
      ],
    },
    children: [
      {
        name: 'liveRoomList',
        path: '/liveRoomManage/liveRoomList',
        component: () => import('@/views/liveRoom/list/index.vue'),
        meta: {
          title: 'router.liveRoomList',
        },
      },
      {
        name: 'liveRoomUpdate',
        path: '/liveRoomManage/liveRoomUpdate',
        component: () => import('@/views/liveRoom/update/index.vue'),
        meta: {
          title: 'router.liveRoomUpdate',
        },
      },
    ],
  },
  {
    path: '/msgManage',
    component: Layout,
    meta: {
      title: 'router.msgManage',
      icon: renderIcon(PaperPlaneOutline),
      sort: ROUTE_SORT.live,
      roles: [
        DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value,
        DEFAULT_ROLE_INFO.LIVE_ADMIN.role_value,
      ],
    },
    children: [
      {
        name: 'msgList',
        path: '/msgManage/msgList',
        component: () => import('@/views/msg/list/index.vue'),
        meta: {
          title: 'router.msgList',
        },
      },
      {
        name: 'msgUpdate',
        path: '/msgManage/msgUpdate',
        component: () => import('@/views/msg/update/index.vue'),
        meta: {
          title: 'router.msgUpdate',
        },
      },
    ],
  },
  {
    name: 'user',
    path: '/user',
    component: Layout,
    meta: {
      title: 'router.user',
      icon: renderIcon(PersonOutline),
      sort: ROUTE_SORT.user,
      roles: [
        DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value,
        DEFAULT_ROLE_INFO.ADMIN.role_value,
      ],
    },
    children: [
      {
        name: 'userList',
        path: '/user/list',
        component: () => import('@/views/user/list/index.vue'),
        meta: {
          title: 'router.userList',
          roles: [
            DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value,
            DEFAULT_ROLE_INFO.ADMIN.role_value,
          ],
        },
      },
      {
        name: 'userAdd',
        path: '/user/add',
        component: () => import('@/views/user/add/index.vue'),
        meta: {
          title: 'router.userAdd',
          roles: [DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value],
        },
      },
      {
        name: 'invite',
        path: '/user/invite',
        component: () => import('@/views/user/invite/index.vue'),
        meta: {
          title: 'router.userInvite',
          roles: [
            DEFAULT_ROLE_INFO.ADMIN.role_value,
            DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value,
          ],
        },
      },
    ],
  },
  {
    path: '/role',
    component: Layout,
    meta: {
      title: 'router.role',
      icon: renderIcon(LockClosedOutline),
      sort: ROUTE_SORT.role,
      roles: [ROUTE_ROLES.SUPER_ADMIN],
    },
    children: [
      {
        name: 'roleList',
        path: '/role/list',
        component: () => import('@/views/role/list/index.vue'),
        meta: {
          title: 'router.roleList',
        },
      },
      {
        name: 'roleTree',
        path: '/role/tree',
        component: () => import('@/views/role/tree/index.vue'),
        meta: {
          title: 'router.roleTree',
        },
      },
      {
        name: 'addRole',
        path: '/role/add',
        component: () => import('@/views/role/add/index.vue'),
        meta: {
          title: 'router.addRole',
        },
      },
    ],
  },
  {
    name: 'auth',
    path: '/auth',
    component: Layout,
    meta: {
      title: 'router.auth',
      icon: renderIcon(ShieldOutline),
      sort: ROUTE_SORT.auth,
      roles: [ROUTE_ROLES.SUPER_ADMIN],
    },
    children: [
      {
        name: 'authList',
        path: '/auth/list',
        component: () => import('@/views/auth/list/index.vue'),
        meta: {
          title: 'router.authList',
        },
      },
      {
        name: 'authTree',
        path: '/auth/tree',
        component: () => import('@/views/auth/tree/index.vue'),
        meta: {
          title: 'router.authTree',
        },
      },
      {
        name: 'addAuth',
        path: '/auth/add',
        component: () => import('@/views/auth/add/index.vue'),
        meta: {
          title: 'router.addAuth',
        },
      },
    ],
  },
  {
    name: 'order',
    path: '/order',
    component: Layout,
    meta: {
      icon: renderIcon(WalletOutline),
      title: 'router.order',
      sort: ROUTE_SORT.order,
      roles: [DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value],
    },
    children: [
      {
        name: 'orderList',
        path: '/order/list',
        component: () => import('@/views/order/list/index.vue'),
        meta: {
          title: 'router.orderList',
        },
      },
    ],
  },
  {
    name: 'goods',
    path: '/goods',
    component: Layout,
    meta: {
      icon: renderIcon(BagHandleOutline),
      title: 'router.goods',
      sort: ROUTE_SORT.goods,
      roles: [DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value, 'qyp'],
    },
    children: [
      {
        name: 'goodsList',
        path: '/goods/list',
        component: () => import('@/views/goods/list/index.vue'),
        meta: {
          title: 'router.goodsList',
        },
      },
      {
        name: 'goodsCreate',
        path: '/goods/create',
        component: () => import('@/views/goods/create/index.vue'),
        meta: {
          title: 'router.goodsCreate',
        },
      },
      {
        name: 'goodsUpdate',
        path: '/goods/update',
        component: () => import('@/views/goods/update/index.vue'),
        meta: {
          title: 'router.goodsUpdate',
          hidden: true,
        },
      },
    ],
  },
  {
    name: 'ad',
    path: '/ad',
    component: Layout,
    meta: {
      icon: renderIcon(ListCircleOutline),
      title: 'router.ad',
      sort: ROUTE_SORT.ad,
      roles: [DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value],
      auths: [DEFAULT_AUTH_INFO.AD_MANAGEMENT.auth_value],
    },
    children: [
      {
        name: 'adList',
        path: '/ad/index',
        component: () => import('@/views/ad/index.vue'),
        meta: {
          title: 'router.adList',
        },
      },
    ],
  },
  {
    name: 'wallet',
    path: '/wallet',
    component: Layout,
    meta: {
      icon: renderIcon(ListCircleOutline),
      title: 'router.wallet',
      sort: ROUTE_SORT.wallet,
      auths: [DEFAULT_AUTH_INFO.WALLET_MANAGEMENT.auth_value],
    },
    children: [
      {
        name: 'backConfig',
        path: '/wallet/backConfig',
        component: () => import('@/views/wallet/backConfig/index.vue'),
        meta: {
          title: 'router.backConfig',
          auths: [DEFAULT_AUTH_INFO.BANK_CONFIG.auth_value],
        },
      },
      {
        name: 'userBankCard',
        path: '/wallet/userBankCard',
        component: () => import('@/views/wallet/userBankCard/index.vue'),
        meta: {
          title: 'router.userBankCard',
          auths: [DEFAULT_AUTH_INFO.BANK_CARD_MANAGEMENT.auth_value],
        },
      },
      {
        name: 'rechargeRecord',
        path: '/wallet/rechargeRecord',
        component: () => import('@/views/wallet/rechargeRecord/index.vue'),
        meta: {
          title: 'router.rechargeRecord',
          auths: [DEFAULT_AUTH_INFO.RECHARGE_RECORD.auth_value],
        },
      },
      {
        name: 'withdrawalRecords',
        path: '/wallet/withdrawalRecords',
        component: () => import('@/views/wallet/withdrawalRecords/index.vue'),
        meta: {
          title: 'router.withdrawalRecords',
          auths: [DEFAULT_AUTH_INFO.WITHDRAWAL_RECORD.auth_value],
        },
      },
    ],
  },
  {
    name: 'quiz',
    path: '/quiz',
    component: Layout,
    meta: {
      icon: renderIcon(ListCircleOutline),
      title: 'router.quiz',
      sort: ROUTE_SORT.quiz,
      auths: [DEFAULT_AUTH_INFO.MATCH_MANAGE.auth_value],
    },
    children: [
      {
        name: 'quizList',
        path: '/quiz/list',
        component: () => import('@/views/quiz/list/index.vue'),
        meta: {
          title: 'router.quizList',
          auths: [DEFAULT_AUTH_INFO.MATCH_LIST.auth_value],
        },
      },
      {
        name: 'quizVotes',
        path: '/quiz/votes/list',
        component: () => import('@/views/quiz/votes/index.vue'),
        meta: {
          title: 'router.quizVotes',
          auths: [DEFAULT_AUTH_INFO.MATCH_VOTES.auth_value],
        },
      },
      {
        name: 'quizSeasons',
        path: '/quiz/seasons/list',
        component: () => import('@/views/quiz/season/index.vue'),
        meta: {
          title: 'router.quizSeasons',
          auths: [DEFAULT_AUTH_INFO.MATCH_SEASONS_LIST.auth_value],
        },
      },
      {
        name: 'quizType',
        path: '/quiz/type',
        component: () => import('@/views/quiz/type/index.vue'),
        meta: {
          title: 'router.quizType',
          roles: [DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value],
          auths: [DEFAULT_AUTH_INFO.MATCH_TYPES.auth_value],
        },
      },
      {
        name: 'quizTeam',
        path: '/quiz/team',
        component: () => import('@/views/quiz/team/index.vue'),
        meta: {
          title: 'router.quizTeam',
          auths: [DEFAULT_AUTH_INFO.MATCH_TEAMS.auth_value],
        },
      },
    ],
  },
  // 新增记账记录菜单
  {
    name: 'accountRecord',
    path: '/accountRecord',
    component: Layout,
    meta: {
      icon: renderIcon(ListCircleOutline),
      title: 'router.accountRecord',
      sort: ROUTE_SORT.accountRecord, // 使用常量而不是硬编码值
      auths: [DEFAULT_AUTH_INFO.AGENCY_BOOKKEEPING.auth_value],
    },
    children: [
      {
        name: 'quizPayouts',
        path: '/accountRecord/payouts',
        component: () => import('@/views/accountRecord/payouts/index.vue'),
        meta: {
          title: 'router.profitStatistics',
          auths: [DEFAULT_AUTH_INFO.PROFIT_BOOKKEEPING.auth_value],
        },
      },
      {
        name: 'gameConsumptionRecord',
        path: '/accountRecord/agentUserRecords',
        component: () =>
          import('@/views/accountRecord/agentUserRecords/index.vue'),
        meta: {
          title: 'router.gameConsumptionRecord',
          auths: [DEFAULT_AUTH_INFO.GAME_CONSUMPTION_BOOKKEEPING.auth_value],
        },
      },
      {
        name: 'memberRecords',
        path: '/accountRecord/memberRecords',
        component: () =>
          import('@/views/accountRecord/memberRecords/index.vue'),
        meta: {
          title: 'router.memberRecords',
          auths: [DEFAULT_AUTH_INFO.MEMBER_BOOKKEEPING.auth_value],
        },
      },
    ],
  },
  {
    name: 'loginRecord',
    path: '/loginRecord',
    component: Layout,
    meta: {
      icon: renderIcon(FootstepsOutline),
      title: 'router.loginRecord',
      sort: ROUTE_SORT.loginRecord,
      roles: [DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value],
    },
    children: [
      {
        name: 'loginRecordList',
        path: '/loginRecord/list',
        component: () => import('@/views/loginRecord/list/index.vue'),
        meta: {
          title: 'router.loginRecordList',
        },
      },
    ],
  },
  {
    name: 'globalMsg',
    path: '/globalMsg',
    component: Layout,
    meta: {
      icon: renderIcon(ChatbubblesOutline),
      title: 'router.globalMsg',
      sort: ROUTE_SORT.globalMsg,
      roles: [DEFAULT_ROLE_INFO.SUPER_ADMIN.role_value],
    },
    children: [
      {
        name: 'globalMsgList',
        path: '/globalMsg/list',
        component: () => import('@/views/globalMsg/list/index.vue'),
        meta: {
          title: 'router.globalMsgList',
        },
      },
      {
        name: 'globalMsgCreate',
        path: '/globalMsg/create',
        component: () => import('@/views/globalMsg/create/index.vue'),
        meta: {
          title: 'router.globalMsgCreate',
        },
      },
      {
        name: 'globalMsgUpdate',
        path: '/globalMsg/update',
        component: () => import('@/views/globalMsg/update/index.vue'),
        meta: {
          title: 'router.globalMsgUpdate',
          hidden: true,
        },
      },
    ],
  },
  ...notFoundRoutes, // 404路由一定要放在异步路由的最后
];
