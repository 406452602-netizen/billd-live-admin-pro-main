      ...
      >
        ...
        >
          ...
          <template v-else-if="item.type === FormTypeEnum.datePickerDatetime">
            <n-date-picker
              :value="modelValue[`${item.field}`]"
              :is-date-disabled="item.isDateDisable"
              type="datetime"
              clearable
              @update:value="handleValueChange($event, item.field)"
            ></n-date-picker>
...
import UploadCpt from '@/components/Base/BaseUpload';
  ...
  try {
    ...
    results.forEach((result, index) => {
      if (result.status === 'fulfilled') {
        ...
        if (value.flag) {
          successfulUploads.push(value);
          res[value.field] = value.result;
          del.push(...value.del);
        } else {
          console.error('上传失败:', value.error);
          window.$message.error(`文件 ${index + 1} 上传失败`);
        }
      } else {
        console.error('上传任务出错:', result.reason);
        window.$message.error(`文件 ${index + 1} 上传异常`);
      }
    });
